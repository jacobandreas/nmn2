...
if __name__ == "__main__":
  ...
  for i in range(40000):
    ...
    for query, data in val_data_by_query.items():
      ...
      query_size = str(query).count("(")
      epoch_acc_by_size[query_size] += acc_here
      epoch_counts_by_size[query_size] += 1

    if val_data_by_query:
        epoch_val_ll /= len(val_data_by_query)
        epoch_val_acc /= len(val_data_by_query)

        for size in epoch_acc_by_size:
            epoch_acc_by_size[size] /= epoch_counts_by_size[size]
